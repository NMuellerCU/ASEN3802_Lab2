function [u_cell] = calc_u_model3(n,H,M,T_0,x,time,const,material_num,model_num,t_step,alpha)
    ...
    % Initializing u_cell
    u_cell = cell(size(alpha));

    for a = 1:length(alpha)
        for i = 1:length(material_num) % Cycling through each case (1 = Alum 25V)
            ...
            for t = 0:height(temp_mat)
                ...
                for k = 1:length(x)
                    ...
                    temp_mat(t+1,k+1) = calcSum_u(n,H(i),M(i),T_0(i),x(k),temp_mat(t+1,1),const,material_num(i),model_num,alpha(a,i));
                end
            end
